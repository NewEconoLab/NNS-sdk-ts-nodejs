import { Neo } from "nel-thinsdk-ts";
...
export class www
{
    ...
    {
        var postdata = www.makeRpcPostBody("sendrawtransaction", Neo.Func.toHexString(data));
        var result = await fetch(www.api, { "method": "post", "body": JSON.stringify(postdata) });
        var json = await result.json();
        if (json[ "result" ])
        {
    ...
    {
        var str = www.makeRpcUrl(www.api, "getstorage", Neo.Func.toHexString(scripthash), Neo.Func.toHexString(key));
        var result = await fetch(str, { "method": "get" });
        var json = await result.json();
        if (json[ "result" ] == null)
            return null;
    ...
    {
        var str = www.makeRpcUrl(www.api, "invokescript", Neo.Func.toHexString(scripthash));
        var result = await fetch(str, { "method": "get" });
        var json = await result.json();
        if (json[ "result" ] == null)
            return null;
    ...
    {
        var postdata = www.makeRpcPostBody("rechargeandtransfer", Neo.Func.toHexString(data1), Neo.Func.toHexString(data2));
        var result = await fetch(www.apiaggr, { "method": "post", "body": JSON.stringify(postdata) });
        var json = await result.json();
