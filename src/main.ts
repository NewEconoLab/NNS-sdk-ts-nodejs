...
export async function resolveDomain(
      ...
      const response = await callContract(url, domain, contract, "resolve", args);
...
export async function getNameHash(
      ...
      const response = await callContract(url, domain, contract, "nameHash", args);
...
export async function setResolver(
      ...
      const owner = wallet.getScriptHashFromAddress(address); 
      const tldRegEx = ".".concat(tld).concat("$");
      const regExp = new RegExp(tldRegEx);
      const subdomain = domain.replace(regExp, "");
      const hashSubdomain = u.sha256(u.str2hexstring(subdomain));
      ...
      const resolver = sc.ContractParam.byteArray(resolver, "resolver");

      const args = [ owner, fullhash, resolver ];

      const response = await callContract(url, domain, contract, "owner_SetResolver", args);
...
export async function setAddress(
      ...
      const response = await callContract(url, domain, contract, "setResolverData", args);
